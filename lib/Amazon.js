...
class AmazonScraper {
     ...
     */
    grabProduct(body, p) {
        fs.writeFileSync(path.resolve(__dirname, '../', 'output.html'), body);
        const $ = cheerio.load(body.replace(/\s\s+/g, '').replace(/\n/g, ''));
        ...
        for (let key in scrapingResult) {
            try {
                ...
                if (titleThumbnailSearch) {
                    ...
                    const srcSet = titleThumbnailSearch.attribs.srcset.split(',').map((i) => i.trim());
                    scrapingResult[key].srcSet = srcSet[srcSet.length - 1].split(' ')[0];
                }
            } catch (err) {
                continue;
            }
